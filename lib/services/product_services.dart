...
//   }
  ...
  Future deleteProduct(id) async {
    var response = await http.get(Uri.parse('${CONTANTS.BASE_DOMAIN}/api/product/delete/$id'), headers: headers(_box));
  ...
  Future deleteCatelog(id) async {
    var response = await http.get(Uri.parse('${CONTANTS.BASE_DOMAIN}/api/category/delete/$id'), headers: headers(_box));
    if (response.statusCode == 200) {
      if (response.body.isNotEmpty) {
        var result = jsonDecode(response.body);
        if (result['alert'] == 'success') {
