...
class PaymentController extends GetxController {
  ...
  saveOrder() async {
    ...
    if (result != null) {
      Get.offAll(PaymentDoneScreen(),
          arguments: {'totalPrice': totalPrice.value, 'amountReceive': 0, 'icon': MdiIcons.clock, 'order_id': result['id'], 'status_title': 'common.added_to_order'.tr, 'color': Colors.grey});
    }
  }
  ...
  paymentOrder(totalPrice, amountReceive) async {
    ...
    if (result != null) {
      Get.back();
      Get.offAll(PaymentDoneScreen(),
          arguments: {'totalPrice': totalPrice, 'amountReceive': (amountReceive), 'icon': MdiIcons.check, 'order_id': result['id'], 'status_title': 'common.payment_success'.tr, 'color': Colors.green});
    }
  }
