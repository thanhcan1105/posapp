...
class AnalyticController extends GetxController with SingleGetTickerProviderMixin {
  ...
  mapResponseToIndex() {
    ...
    if (reportLists.length != 0) {
      ...
      var groupMapRevenueByDate = groupBy(mapRevenueByDate, (Map obj) => DateFormat('yyyy-MM-dd').format(obj['createdAt']));
      mapSaleByDate.clear();
      ...
      //best product
      List<Map<dynamic, dynamic>> getListProduct = reportLists.map((v) {
      ...
      var groupListProduct = groupBy(getListProduct, (Map obj) => obj['productId']);
      var maxProduct = 0;
      groupListProduct
        ..forEach((k, v) {
      ...
      if (inDays == 0 || inDays == -1) {
        groupRevenueByHour = groupBy(mapRevenueByHour, (Map obj) => DateFormat('yyyy-MM-dd HH').format(obj['createdAt']));
      } else {
        groupRevenueByHour = groupBy(mapRevenueByHour, (Map obj) => DateFormat('HH').format(obj['createdAt']));
      }
      ...
      groupRevenueByEmployee = groupBy(reportLists, (ReportModel obj) => obj.user?.id);
      mapRevenueByUser.clear();
      groupRevenueByEmployee.forEach((k, v) {
        mapRevenueByUser.add({'user_id': k, 'user_name': reportLists.value.firstWhere((e) => e.user!.id == k).user!.name, 'revenue': v.map((e) => e.totalPrice).reduce((a, b) => a + b), 'sale': v.length, 'max': false, 'percent': ((v.length) / (sales.value) * 100).toStringAsFixed(0)});
      });
  ...
  String get typeFilterLabel {
    switch (typeFilter.value) {
      case 'today':
        return 'time.today'.tr + ' (${DateFormat('dd-MM').format(DateTime.now())})';
        break;
      case 'this_week':
        return 'time.this_week'.tr;
        break;
      case 'this_month':
        return 'time.this_month'.tr + ' (tháng ${DateFormat('MM').format(DateTime.now())})';
        break;
      case 'this_year':
        return 'time.this_year'.tr + ' (${DateFormat('yyyy').format(DateTime.now())})';
        break;
      case 'yesterday':
        return 'time.yesterday'.tr + ' (${DateFormat('dd-MM').format(DateTime.now().subtract(Duration(days: 1)))})';
        break;
      case 'last_week':
        return 'time.last_week'.tr;
        break;
      case 'last_month':
        return 'time.last_month'.tr + ' (tháng ${DateFormat('MM').format(DateTime.now().subtract(Duration(days: 30)))})';
        break;
      case 'last_year':
        return 'time.last_year'.tr + ' (${DateFormat('yyyy').format(DateTime.now().subtract(Duration(days: 365)))})';
        break;
